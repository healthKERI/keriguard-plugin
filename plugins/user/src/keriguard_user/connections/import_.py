# -*- encoding: utf-8 -*-
"""keriguard_user.connections.import_ — Import connection credential from .cesr file."""
...
class ImportConnectionCredentialPage(LocksmithFormPage):
    """Import a connection credential from a .cesr grant file."""
    ...
    def __init__(self, app: "LocksmithApplication", parent: "VaultPage | None" = None):
        super().__init__(
            title="Import Connection Credential",
    ...
    def _build_content(self):
        ...
        self._add_section_header(
            layout,
            header="Import Connection Credential",
            sub=(
                "Select a .cesr file exported by the KERIGuard admin. "
                "The credential will be loaded into your vault and the peer's WireGuard "
                "configuration will be added automatically."
            ),
        )
    ...
    async def _on_import(self):
        ...
        try:
            ...
            logger.exception(f"ImportConnectionCredentialPage: import failed: {exc}")
    ...
    async def _run_import(self, path: str):
        ...
        # Find a local hab to use for parsing (first available)
        hab = next(iter(vault.hby.habs.values()), None)
        if hab is None:
            self._show_status("No identifier found in this vault.", error=True)
            return
